...
{
    // Modified "DirectionalIndicator.cs"
    // Difference: Measures "passedTime" the target object has been "seen".
    // If this time supasses a specified "saturationTime" the indicator is disabled/target removed
    // If this time supasses a specified "saturationTime" the indicator is disabled/target removed
    public class CustomDirectionalIndicator : Solver
    {
        [Tooltip("Total time the target object has to be considered as seen to then disable solvers (in seconds)")]
        public float saturationTime = 1.5f;
        ...
        private void Update()
        {
            using (UpdatePerfMarker.Auto())
            {
                ...
                {
                    // Disable solvers if target object has been seen for specified saturation
                    if (passedTime > saturationTime)
                    {
                        // Remove target object from indication
                        DirectionalTarget = null;

                        // Reset vars for next tracking
                        passedTime = 0f;
                    }

                    // Otherwise just increment passed time
        ...
        private void SetIndicatorVisibility(bool showIndicator)
        {
            ...
            GetComponentsInChildren(childRenderers);
