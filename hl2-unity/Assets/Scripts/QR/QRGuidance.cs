...
public class QRGuidance
    : MonoBehaviour
{
    public static QRGuidance instance;
    ...
    [Tooltip("List of positions of qr code recorded during tracking")]
    private List<Vector3> trackingPositions;
    [Tooltip("List of rotations of qr code recorded during tracking")]
    private List<Quaternion> trackingRotations;
        ...
        foreach (ARMarker qrCode in args.updated)
        {
            ...
            if (arMarkerManager.GetDecodedString(qrCode.trackableId) == payloadQr1)
            {
                ...
                if (trackingCounter >= trackingSaturationCounter)
                {
                    ...
                    {
                        // Update UI (Only if initial tracking)
                        qrcodePanel.SetActive(false);
                        UIGuidance.instance.handmenu.SetActive(true);
                        UIGuidance.instance.fovCheck.SetActive(true);
                        UIGuidance.instance.fovConfirmButton.SetActive(true);
                    }
                }
            }
        }
